...
# (HR identifier, HD identifier, RA and DEC coordinates, B magnitude, V magnitude, spectral type, b-y color index,
#  m1 color index, c1 color index and Hbeta index) and outputs an Excel file with the
#  information requested (optional)
#Written by Jorge Lozano
#Last_time_updated: 05/19/2020
...
#Method: Inputs string of idenfiers and outputs a list containing each identifier
...
#Method:  (Helper for request_df ()  ) inputs the "IDS" column from dataframe and returns 2 lists: HR identifiers, HD  identifiers 
...
def request_df (list_identifiers ): 
    ...
    customSimbad.add_votable_fields('ids','plx','flux(B)','flux(V)','sptype')    
    ...
    simbad_df = simbad_df.filter(['IDS', 'RA','DEC', 'PLX_VALUE', 'FLUX_B', 'FLUX_V', 'SP_TYPE',])
    ...
    #Joining Vizier and Simbad information in one dataframe
...
def write (datafr, file_name): 
    ...
    worksheet.set_column('A:M',14) #14 sets column width
    writer.save()
    #datafr.to_excel(file_name,sheet_name='script_output',index=False)
...
def main ():
    ...
    my_df = my_df.replace(['NaN', np.nan,'nan'], '', regex=True) #replaces NaN values for an empty string
    #Changing column names
    # my_df.columns=['HR','HD','RA','DEC','Parallax','B','V','SpT','Quality','uvby98 V','b-y','m1','c1','Hbeta']
    my_df.columns=['HR','HD','RA','DEC','Parallax','B','V','SpT','uvby98 V','b-y','m1','c1','Hbeta']
